...
pub async fn eliminar_usuario_por_id(
    ...
    match UsuarioStreamingRepository::eliminar(&pool, id_usuario).await {
        Ok(_) => Json(true),
        Err(_) => Json(false),
    }
}
...
pub async fn actualizar_solo_suscripcion(
    State(pool): State<PgPool>,
    Path(id): Path<i32>,
    Json(nueva_suscripcion): Json<String>,
) -> Json<UsuarioStreaming> {
    let datos_actualizar = ActualizarUsuario {
        nombre_usuario: None,
        tipo_suscripcion: Some(nueva_suscripcion),
    };
        ...
        .await
    {
        ...
        Err(_) => Json(UsuarioStreaming {
            id_usuario: 0,
            nombre_usuario: "Error al actualizar la suscripción".to_string(),
            tipo_suscripcion: "Free".to_string(),
        }),
    }
}
