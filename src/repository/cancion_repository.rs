...
impl CancionRepository {
    ...
    pub async fn obtener_canciones(&self) -> sqlx::Result<Vec<Cancion>> {
        let filas =
            sqlx::query("SELECT id_cancion, nombre, duracion::TEXT, id_album FROM canciones")
                .fetch_all(&self.pool)
                .await?;
        let canciones = filas
            .into_iter()
    ...
    pub async fn obtener_cancion_por_id(&self, id: i32) -> sqlx::Result<Cancion> {
        let fila = sqlx::query("SELECT id_cancion, nombre, duracion::TEXT, id_album FROM canciones WHERE id_cancion = $1")
            .bind(id)
            .fetch_one(&self.pool)
            .await?;
        Ok(Cancion {
    ...
    pub async fn agregar_cancion(&self, nueva: NuevaCancion) -> sqlx::Result<Cancion> {
        let fila = sqlx::query("INSERT INTO canciones (nombre, duracion, id_album) VALUES ($1, $2::TIME, $3) RETURNING id_cancion, nombre, duracion::TEXT, id_album")
            .bind(nueva.nombre)
            .bind(nueva.duracion)
            .bind(nueva.id_album)
            .fetch_one(&self.pool)
    ...
    pub async fn actualizar_cancion(
        ...
        let fila = sqlx::query("UPDATE canciones SET nombre = COALESCE($1, nombre), duracion = COALESCE($2::TIME, duracion), id_album = COALESCE($3, id_album) WHERE id_cancion = $4 RETURNING id_cancion, nombre, duracion::TEXT, id_album")
            .bind(datos.nombre)
            .bind(datos.duracion)
            .bind(datos.id_album)
            .bind(id)
